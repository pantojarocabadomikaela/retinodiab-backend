"""
Inferencia para P2 Green + CLAHE — Fold 2.

Django continúa llamando simplemente a predict_image(image_bytes).
"""
...
def predict_image(image_bytes: bytes) -> InferenceResult:
    ...
    tensor = tensor.to(device=device, dtype=torch.float32, non_blocking=False)
    ...
    probabilities_cpu = probabilities_tensor[0].detach().to(
        device="cpu", dtype=torch.float64
    )
    logits_cpu = logits_tensor[0].detach().to(
        device="cpu", dtype=torch.float64
    )
    ...
    if abs(probability_sum - 1.0) > 1e-6:
        raise InferenceError(
            f"Softmax inválido: suma={probability_sum:.12f}."
        )

    class_id = int(torch.argmax(probabilities_cpu).item())
    if class_id not in CLASS_NAMES:
        raise InferenceError(f"Clase fuera del mapping: {class_id}.")

    return InferenceResult(
        model_id=MODEL_ID,
        class_id=class_id,
        label=CLASS_NAMES[class_id],
        probabilities=[float(v) for v in probabilities_cpu.tolist()],
        logits=[float(v) for v in logits_cpu.tolist()],
        device=str(device),
    )
